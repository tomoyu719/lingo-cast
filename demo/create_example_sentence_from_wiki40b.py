...
class CreateExampleSentence():
    ...
    # def _get_word_around(self, word, sentences) -> list:
    ...
    #             if self._is_exclude_pos(prev_pos):
    #                 prev = None
    #         except IndexError:
    #             prev = None
    #         try:
    ...
    #             if self._is_exclude_pos(next_pos):
    #                 next = None
    #         except IndexError:
    #             prev = None
    #         if prev and next:
    #             short_sentences_contain_word.append(prev + ' ' + word + ' ' + next)
            
            # short_sentence = word
            # for i in range(index - 1, 0-1, -1):
            #     prev = split[i]
            #     pos = self.morph.get_word_pos(prev)
            #     if self._is_exclude_pos(pos):
            #         break
    ...
    def make_example_sentence(self, word) -> str:
        ...
        # sentence_with_score = sorted(sentence_with_score.items(),key=lambda item: item[1], reverse=True)
